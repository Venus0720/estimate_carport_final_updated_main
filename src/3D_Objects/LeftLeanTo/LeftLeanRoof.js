...
export default class LeftLeanToRoof extends Component {
    
    
  createLeftLeanToRoof = (const_var,params)=>{
    ...
    let lRoofGeometry1 = new THREE.BufferGeometry();
    ...
    let lRoofmaterial= new THREE.MeshPhongMaterial({ 
    ...
    let lRoof = new THREE.Mesh( lRoofGeometry1, lRoofmaterial );
    lRoof.name = "lRoof";
    lRoof.visible = false;	
	...
	var verticesT = [
	  ...
	  // var materialT = new THREE.MeshBasicMaterial({ map: textureT,side:THREE.DoubleSide, color:0xFFFF00});
	  ...
	  cubeTLL.visible = false;
	  var rCalor1 = params.p_r_c.replace("#","0x");
	  cubeTLL.material.color.setHex( rCalor1 );
	...
	let leanTohInc = roofMiddleHeight[params.b_l_t_r_p][2*params.lean_p_w]/2 - 0.1;

	//Bow for left leanto in front position
	let leftLeanToFrontBow = new THREE.Mesh(bowGeometry,bowMaterial);
	...
	leftLeanToFrontBow.visible = false;
	const_var.b_t_M_L_t_L.add(leftLeanToFrontBow);

	// let leftLeanToRegularBowGeo = new THREE.Shape();
    // leftLeanToRegularBowGeo.moveTo(params.p_w/4, params.p_h+((params.p_r_p/12)*params.p_w/4)); //bottom left
    // leftLeanToRegularBowGeo.lineTo(params.p_w/4, params.p_h+((params.p_r_p/12)*params.p_w/4)+5);//top left
    // leftLeanToRegularBowGeo.lineTo(params.p_w/4+0.01, params.p_h+((params.p_r_p/12)*params.p_w/4)+5); // top right
    // leftLeanToRegularBowGeo.lineTo(params.p_w/4+0.01, params.p_h+((params.p_r_p/12)*params.p_w/4)); // bottom right
    // leftLeanToRegularBowGeo.lineTo(params.p_w/4, params.p_h+((params.p_r_p/12)*params.p_w/4));
    // let extrudeSettingsLeftLeanToRegularBow = {
    //     depth: -params.p_d, // height of the raft
    //     bevelEnabled: false, // no bevel on edges
    //     };
    //     let leftLeanToRegularBowGeometry = new THREE.ExtrudeGeometry(
    //         leftLeanToRegularBowGeo,
    //         extrudeSettingsLeftLeanToRegularBow
    //     );
    // let leftLeanToRegularBow = new THREE.Mesh(leftLeanToRegularBowGeometry, bowMaterial);
    // leftLeanToRegularBow.position.z = params.p_d/2;
    // leftLeanToRegularBow.name = "leftLeanToRegularBow"
    // const_var.b_t_M_L_t_L.add(leftLeanToRegularBow);

	//Bow for left leanto regular in front position
	let leftLeanToFrontBowR = new THREE.Mesh(bowGeometry,bowMaterial);
	...
	leftLeanToFrontBowR.visible = false;
	...
	let leanToleftFrontBow1 = new THREE.Mesh(leftBowGeo1,bowMaterial);
	...
	leanToleftFrontBow1.visible = false; 
    ...
    let leanToleftFrontBow2 = new THREE.Mesh(leftBowGeo2,bowMaterial);
    ...
    leanToleftFrontBow2.visible = false; 
	...
	let leanToleftFrontBow3 = new THREE.Mesh(leftBowGeo3,bowMaterial);
	leanToleftFrontBow3.name = "leanToleftFrontBow3";
	...
	leanToleftFrontBow3.visible = false; 
	const_var.b_t_M_L_t_L.add(leanToleftFrontBow3);

    //corner for left leanto in front position
	let leftFrontCornerGeo = new THREE.BoxGeometry(0.98,0.6+0.2,0.2+0.1);
	let leanToleftFrontCorner = new THREE.Mesh(leftFrontCornerGeo,bowMaterial);
	...
	leanToleftFrontCorner.visible = false; 
	const_var.b_t_M_L_t_L.add(leanToleftFrontCorner);

	//corner for left leanto regular in front position
	let leftBackCornerGeo = new THREE.BoxGeometry(0.98,0.6+0.2,0.2+0.1);
	let leanToleftFrontCornerR = new THREE.Mesh(leftBackCornerGeo,bowMaterial);
	...
	leanToleftFrontCornerR.visible = false; 
	const_var.b_t_M_L_t_L.add(leanToleftFrontCornerR);


     //Left Leanto Front Trim For A-frame and Vertical
	 ...
	 let leftLeanToFrontTrimA = new THREE.Mesh(trimGeometry,bowMaterial1);
	 leftLeanToFrontTrimA.name = "leftLeanToFrontTrimA";
	 leftLeanToFrontTrimA.visible = false; 
	 const_var.b_t_M_L_t_L.add(leftLeanToFrontTrimA);

	 //Left Leanto Left Trim For A-frame and Vertical
	 let leanToleftlegGeo = new THREE.BoxGeometry(0.31+0.1, 1+0.0015, 0.2+0.07);
	 let leftLeantoLeftTrimA = new THREE.Mesh(leanToleftlegGeo,bowMaterial1);
	 leftLeantoLeftTrimA.name = "leftLeantoLeftTrimA";
	 leftLeantoLeftTrimA.visible = false; 
	 const_var.b_t_M_L_t_L.add(leftLeantoLeftTrimA);
}
    render() {
