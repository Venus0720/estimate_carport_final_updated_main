...
export default class FrontLeanToRoof extends Component {

    createFrontLeanToRoof = (const_var,params)=>{
    	...
    	let posBow2 = {
    	    "1":{"12": 11.896,"13": 12.896, "14":13.896, "15":14.896, "16":15.896, "17":16.896, "18":17.896, "19":18.896, "20":19.896, "21":20.896, "22":21.896, "23":22.896, "24":23.896},
        ...
        let widthBowR = {
    		"1": {"12": 0.18,"13": 0.21, "14":0.25, "15":0.29, "16":0.33, "17":0.36, "18":0.39, "19":0.42, "20":0.45, "21":0.49, "22":0.52, "23":0.57, "24":0.6},
    	...
    	let rotBow1 = {"3":0.5 , "2":0.415, "4":0.6, "5":0.65};
    	let posBow1 = {"3":0.05, "2":0.055,"4":0.044,"5":0.04};
        //scale and position of Bow2 acording to roof pitch
    	let scaleBow2 = {"2":0.23, "3":0.23, "4":0.22, "5":0.22};
        let posCorner = {"1":0.76,"2":0.68, "3":0.605, "4":0.51, "5":0.43, "6":0.35}
            
        let scaleTrim2 ={"1":2.2,"2":2.05, "3":1.9, "4":1.9, "5":1.9, "6":1.9}
        let posTrim2 = {"2":0.17,"3":0.17 ,  "4":0.174, "5":0.174}
        let rotTrim2 = {
        ...
        let trimMaterial1 = new THREE.MeshBasicMaterial({ color: 0x0E6655,side: THREE.DoubleSide});
        ...
        // Front LeanTo Roof
		let quad_vertices = [-0.5,0.0,0.5,   0.0,1.0,0.5,  0.0,1.0,-0.5, -0.5,0.0,-0.5,];
		...
		let fRoofGeometry1 = new THREE.BufferGeometry();
		...
		let fTexture = fRoofloader.load(process.env.REACT_APP_BASE_URL+roofImg, function(texture1) {
		   fTexture.wrapS = THREE.RepeatWrapping;
		   fTexture.wrapT = THREE.RepeatWrapping;
		})
		   let rCalor = params.p_r_c.replace("#","0x");
           let lRoofmaterial= new THREE.MeshPhongMaterial({side: THREE.BackSide,shadowSide:THREE.DoubleSide,
		...
		let fRoof = new THREE.Mesh( fRoofGeometry1, lRoofmaterial );
		fRoof.name = "fRoof";
		fRoof.visible = false;
    ...
    let verticesT = [
      ...
      let materialT= new THREE.MeshPhongMaterial({ 
        ...
        let cubeTFF = new THREE.Mesh(geometryT, materialT);
        cubeTFF.name = "leanToRegularRoofF";
        let rCalorF = params.p_r_c.replace("#","0x");
        cubeTFF.material.color.setHex( rCalorF );
        cubeTFF.visible = false;
        const_var.b_t_M_F_t_F.add(cubeTFF);

        /* Front Lean-TO Right Roof/Bow Trim*/
        let frontLeanToFrontTrim = new THREE.Mesh(bowGeometry,trimMaterial1);
        ...
        frontLeanToFrontTrim.visible = false;
        const_var.b_t_M_F_t_F.add(frontLeanToFrontTrim);

        /* Front Lean-TO Right Trim 1 For Regular Roof Style */
        let leanToFrontTrim1Geo = new THREE.BoxGeometry(0.07+0.007,0.6+0.2,0.2+0.1);
        let leanToLeftFrontTrim1Material = new THREE.MeshBasicMaterial({color:0x0E6655,side:THREE.DoubleSide});
        let frontLeanToFrontTrim1 = new THREE.Mesh(leanToFrontTrim1Geo,leanToLeftFrontTrim1Material);
        ...
        frontLeanToFrontTrim1.visible = false; 
        const_var.b_t_M_F_t_F.add(frontLeanToFrontTrim1);

        /* Front Lean-TO Right Trim 2 For Regular Roof Style */
        let frontLeanToFrontTrim2Geo = new THREE.BoxGeometry(0.07+0.007,0.6+0.2,0.2+0.1);
        let frontLeanToFrontTrim2Material = new THREE.MeshBasicMaterial({color:0x0E6655,side:THREE.DoubleSide});
        let frontLeanToFrontTrim2 = new THREE.Mesh(frontLeanToFrontTrim2Geo,frontLeanToFrontTrim2Material);
        ...
        frontLeanToFrontTrim2.visible = false; 
        const_var.b_t_M_F_t_F.add(frontLeanToFrontTrim2);

        /* Front Lean-TO Right Trim 3 For Regular Roof Style */
        let frontLeanToFrontTrim3Geo = new THREE.BoxGeometry(0.2+0.07,0.6+0.2,0.2+0.1);
        let leanToLeftFrontTrim3Material = new THREE.MeshBasicMaterial({color:0x0E6655,side:THREE.DoubleSide,});
        let frontLeanToFrontTrim3 = new THREE.Mesh(frontLeanToFrontTrim3Geo,leanToLeftFrontTrim3Material);
        ...
        frontLeanToFrontTrim3.visible = false; 
        const_var.b_t_M_F_t_F.add(frontLeanToFrontTrim3);

        /* Front Lean-TO Right Trim 4 For Regular Roof Style */;
        let frontLeanToFrontTrim4 = new THREE.Mesh(frontLeanToFrontTrim3Geo,leanToLeftFrontTrim3Material);
        frontLeanToFrontTrim4.name = "frontLeanToFrontTrim4";
        frontLeanToFrontTrim4.visible = false; 
        const_var.b_t_M_F_t_F.add(frontLeanToFrontTrim4);

        /* Front Lean-TO Right Bow*/
        let frontLeanToFrontBow = new THREE.Mesh(bowGeometry,bowMaterial);
        ...
        frontLeanToFrontBow.visible = false;
        const_var.b_t_M_F_t_F.add(frontLeanToFrontBow);

        /*Front Lean-TO Right Bow For Regular Roof Style*/
        let frontLeanToFrontBowR = new THREE.Mesh(bowGeometry,bowMaterial);
        ...
        frontLeanToFrontBowR.visible = false;
        const_var.b_t_M_F_t_F.add(frontLeanToFrontBowR);

        /*Front Lean-TO Right Curved Bow 1 For Regular Roof Style*/
        let leftBowGeo1 = new THREE.BoxGeometry(0.075,0.6+0.19,0.2+0.1);
        // let leftBowBowMaterial1 = new THREE.MeshBasicMaterial({color:0xC9C6C6,side:THREE.DoubleSide});
        let frontLeanToFrontBow1 = new THREE.Mesh(leftBowGeo1,bowMaterial);
        ...
        frontLeanToFrontBow1.visible = false; 
        const_var.b_t_M_F_t_F.add(frontLeanToFrontBow1);

        /*Front Lean-TO Right Curved Bow 2 For Regular Roof Style*/
        let leftBowGeo2 = new THREE.BoxGeometry(0.075,0.6+0.18,0.2+0.1);
        // let leftBowBowMaterial2 = new THREE.MeshBasicMaterial({color:0xC9C6C6,side:THREE.DoubleSide});
        let frontLeanToFrontBow2 = new THREE.Mesh(leftBowGeo2,bowMaterial);
        ...
        frontLeanToFrontBow2.visible = false; 
        const_var.b_t_M_F_t_F.add(frontLeanToFrontBow2);

        /*Front Lean-TO Right Curved Bow 3 For Regular Roof Style*/
        let leftBowGeo3 = new THREE.BoxGeometry(0.25,0.6+0.23,0.2+0.1);
        // let leftBowBowMaterial3 = new THREE.MeshBasicMaterial({color:0xC9C6C6,side:THREE.DoubleSide,});
        let frontLeanToFrontBow3 = new THREE.Mesh(leftBowGeo3,bowMaterial);
        ...
        frontLeanToFrontBow3.visible = false; 
        const_var.b_t_M_F_t_F.add(frontLeanToFrontBow3);

            /*Front Lean-TO Right Corner */
        let leftCornerGeo = new THREE.BoxGeometry(0.98,0.6+0.2,0.2+0.1);
        // let leftCornerBowMaterial = new THREE.MeshBasicMaterial({color:0xC9C6C6,side:THREE.DoubleSide});
        let frontLeanToFrontCorner = new THREE.Mesh(leftCornerGeo,bowMaterial);
        ...
        frontLeanToFrontCorner.visible = params.add_front_lean; 
        const_var.b_t_M_F_t_F.add(frontLeanToFrontCorner);

        /*Front Lean-TO Right Corner For Regular Roof Style*/
        let leftCornerRGeo = new THREE.BoxGeometry(0.98,0.6+0.2,0.2+0.1);
        // let leftCornerRBowMaterial = new THREE.MeshBasicMaterial({color:0xC9C6C6,side:THREE.DoubleSide});
        let frontLeanToFrontCornerR = new THREE.Mesh(leftCornerRGeo,bowMaterial);
        ...
        frontLeanToFrontCornerR.visible = false; 
        // frontLeanToFrontCornerR.visible = params.add_front_lean; 
        ...
        let frontLeanToRightTrimA = new THREE.Mesh(bowGeometry,trimMaterial);
        frontLeanToRightTrimA.name = "frontLeanToRightTrimA";
        frontLeanToRightTrimA.visible = false;
        const_var.b_t_M_F_t_F.add(frontLeanToRightTrimA);
        
	//Left Leanto Left Trim For A-frame and Vertical
	let leanTofrontlegGeo = new THREE.BoxGeometry(0.31+0.1, 1+0.0015, 0.2+0.07);
	let frontLeantoFrontTrimA = new THREE.Mesh(leanTofrontlegGeo,trimMaterial);
	frontLeantoFrontTrimA.name = "frontLeantoFrontTrimA";
	frontLeantoFrontTrimA.visible = false;
	const_var.b_t_M_F_t_F.add(frontLeantoFrontTrimA);
